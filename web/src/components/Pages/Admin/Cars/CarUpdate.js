...
export function CarUpdate() {
  return (
    <>
      <Head title="Atualizar carro" />
      <div className="container">
        <div className="row">
          <div className="col-md-12">
            <div className="card">
              <div className="card-body text-center">
                <h3>Uma hora eu termino :)</h3>
              </div>
            </div>
          </div>
        </div>
...
import api, { getModels, getCarById } from "~/utils/services/api";
...
export function CarUpdate() {
  ...
  async function loadBrandById(car_id) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (car_km && car_price && car_fuel && car_exchange && car_year && id_model) {
      ...
      try {
        const { data } = await api.put(`/api/car/${car_id}`, {
